...
export const OfficersList = () => {
              ...
              <tbody>
                {personsInfo.map((person, index) => (
                  <tr key={person._id}>
                    ...
                    <td>
                      <button onClick={onApprove} approveperson={index} className='approve'>
                        Approve
                      </button>
                    </td>
                    <td>
                      <button onClick={onDelete} deleteperson={index} className='delete'>
                        Delete
                      </button>
                    </td>
                    <td>
                      <Link className='detail_link' to={`./${person._id}`} state={person}>
  ...
  const renderEmptyState = () => {
    return <p>There are no officers still</p>;
  };

  return <>{personsInfo.length ? renderPersons() : renderEmptyState()}</>;
};
