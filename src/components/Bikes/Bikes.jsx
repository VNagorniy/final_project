...
export const Bikes = () => {
          ...
          cases.map((item, index) => (
            <div className='card' key={index}>
              <p className='item_field'>
                {' '}
                <span className='span'>Full Owner's Name: </span>
                {item.ownerFullName}
              </p>
              <p className='item_field'>
                <span>Status: </span>
                <select defaultValue={'default'} onChange={changeStatus}>
                  <option objectindex={index} value='default'>
                    Set status for your case::
                  </option>
                  <option objectindex={index} value='new'>
                    New
                  </option>
                  <option objectindex={index} value='in_progress'>
                    In Progress
                  </option>
                  <option objectindex={index} value='done'>
                    Finished
                  </option>
                </select>
              </p>
              ...
              <input
                ...
                className='set_message'
                value={resolution}
                placeholder='To make
      resolution, case must be finished'
                onChange={changeResolution}
              ></input>

              <div className='btn-container'>
                <button deteleitem={index} onClick={onDelete} className='btn delete'>
                  Delete
                </button>
                <button className='change' itemindx={index} type='Submit' onClick={onChange} disabled={itemStatus === 'done' && resolution.length === 0}>
                  Save changes
                </button>
  ...
  const renderEmptyState = () => {
    return <p>There are no cases still</p>;
  };

  return <div className='wrapper'>{cases.length ? renderCases() : renderEmptyState()}</div>;
};
